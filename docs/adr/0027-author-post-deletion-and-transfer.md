...
- **Account deletion (amends ADR-0026):** inside the same locked transaction used for the last-active-admin check and comment anonymization, the refusal for "staff with authored posts" narrows to a check against whatever would survive the never-public/commentless-post purge (same predicate as the author self-delete path, scoped to the deleting user) — i.e. posts that were ever public, or that have comments. That check runs, and the delete is refused, **before** the purge itself; only once every refusal check has passed does the purge (and comment anonymization) actually write anything. This ordering is load-bearing, not stylistic: the transaction commits when the callback returns normally and only rolls back on throw, so a refusal returned after a write would still commit that write despite reporting the deletion as refused. The refusal message points the user at the site owner to use the new transfer flow (or an admin hard-delete) to clear those posts before deletion can proceed.

- **Transfer/account-deletion race:** a transfer landing on a user mid-deletion (after their refusal check, before the `user` row delete) is not additionally locked — it's backstopped by the existing FK RESTRICT on `posts.author_id`. If a post lands on the deleting user after the refusal check passes, the row delete fails loudly rather than silently orphaning the post or partially completing the deletion.

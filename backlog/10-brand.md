...
- [ ] **BRAND-2** — Identity assets: SVG logo mark + wordmark in the site header, `src/app/icon.svg`, `apple-icon`, replace scaffold `favicon.ico`, add `manifest.ts` + `themeColor` metadata. _(deps: BRAND-1)_
- [ ] **BRAND-3** — Branded `next/og` share card as the site-wide/fallback OG image (post thumbnails still win per FR-1.4); shrinks SEO-1 to per-post wiring. _(deps: BRAND-1)_

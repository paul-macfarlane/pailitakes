...
- [ ] **CMT-3** — Comment tree UI (TanStack Query): nested render, indent to ~depth 5 then flatten with "replying to @name", mobile-first; optimistic insert on allow. Banned viewer sees an explicit notice in place of the composer ("You're banned from commenting"), not a failing input — they can still read the thread (FR-4.8; §350). _(deps: CMT-2, CMT-4; scope added 2026-07-09 by Paul)_
- [ ] **CMT-4** — Create-comment server action: reject if banned/archived/locked, then rate limit, moderate, insert with resulting status. Rejection returns an explicit reason (banned/archived/locked) the UI can surface — not a generic error swallowed at the boundary. _(deps: CMT-5, CMT-6; scope clarified 2026-07-09 by Paul)_

...
- [ ] **LIKE-2** — Like toggle server actions (insert/delete on composite PK; ban check at action level). Banned tap fails gracefully with a short message (toast/tooltip), not a silent no-op; no site-wide ban banner (§350 — banned users still read normally). _(deps: LIKE-1; scope added 2026-07-09 by Paul)_

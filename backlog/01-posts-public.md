...
- [ ] **POST-5** — Public post page `/posts/[slug]` (ISR, cache tag `post:{slug}`); renders body, associated video, byline, category, tags. _(deps: POST-2, POST-3, POST-6)_
- [ ] **POST-6** — `lite-youtube` click-to-load facade embed component (nocookie). _(deps: FND-1)_
- [ ] **POST-7** — Home page: recent published posts (thumbnail, title, category, author, date, excerpt), ISR `revalidate: 60`, load-more/pagination. Leave a slot for announcements (built in ANN-3). _(deps: POST-2, POST-4)_

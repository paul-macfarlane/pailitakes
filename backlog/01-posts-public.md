...
- [x] **POST-9** — Post banner image: nullable `posts.banner_url` (https image URL, thumbnail fallback), rendered as a hero above the title on `/posts/[slug]`. Cards keep `thumbnail_url`. Admin post form (ADM epic) must include the field. _(deps: POST-5; scope added 2026-07-06 by Paul)_
- [x] **POST-8** — Global nav (home, categories, search) + mobile-first responsive layout shell. _(deps: FND-2)_
- [x] **POST-10** — Post page shows "Updated {date}" when content changed after publish: new nullable `posts.content_updated_at`, stamped only when staged edits are promoted (`promoteStagedDraft`) — NOT driven by `updated_at` (which bumps on every row write, incl. status transitions). Detail page only; cards stay publish-date-only. Needs ADR + design-doc data-model amendment. _(deps: POST-5; scope added 2026-07-11 by Paul via /ask)_

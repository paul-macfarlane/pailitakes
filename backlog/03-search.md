...
- [x] **SRCH-2** — Public category and tag listing pages (ISR, `revalidate: 60`, tag `post-list`), newest first. Category/tag names linkified on cards and post pages; load-more supports filtered feeds. Category browsing later folded into the home page (`/?category=`, pills filter in place — owner feedback 2026-07-11, ADR-0018); tags keep `/tags/[slug]`. _(deps: POST-2)_
...
- [x] **SRCH-5** — Search page: dynamic (never cached), debounced input, results show thumbnail/title/category/date/snippet. GET-form fallback works without JS; params degrade via zod `.catch`. Later folded from `/search` into the home page (`/?q=`, combinable with `?category=` — owner feedback 2026-07-11, ADR-0018). _(deps: SRCH-4)_
